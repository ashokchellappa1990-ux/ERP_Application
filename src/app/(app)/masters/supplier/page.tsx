...
import { type SupplierStatus, type SupplierRow as ListRow } from "@/lib/masters/supplierConfig";
...
const FILTERS: ("All" | SupplierStatus)[] = ["All", "Active", "Inactive", "Blocked"];
const BRANCHES = ["All Branches", "Main Store — MG Road", "Whitefield Branch", "Koramangala Branch"];
...
export default function SupplierMasterPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (j.ok) {
        ...
        setAll((j.suppliers as Record<string, unknown>[]).map((sp) => ({
          id: String(sp.id), code: (sp.code as string) || `SUP-${sp.id}`, name: sp.name as string, type: (sp.type as string) || "—", category: (sp.category as string) || "Supplier",
          gstin: (sp.gstin as string) || "", state: (sp.state as string) || "—", outstanding: (sp.outstanding as number) || 0,
          status: ((sp.status as string) || "Active") as SupplierStatus, preferred: !!sp.preferred, approval: apMap[sp.approvalStatus as string] || "Draft", rating: 0,
        })));
      }
      setLoading(false);
    })();
  ...
  const stats = useMemo(() => ({
    total: all.length,
    active: all.filter((sp) => sp.status === "Active").length,
    blocked: all.filter((sp) => sp.status === "Blocked").length,
    preferred: all.filter((sp) => sp.preferred).length,
    pending: all.filter((sp) => sp.approval === "Pending").length,
    outstanding: all.reduce((s, sp) => s + (sp.outstanding || 0), 0),
  }), [all]);

  const types = useMemo(() => ["All Types", ...Array.from(new Set(all.map((sp) => sp.type)))], [all]);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    return all.filter((sp) => {
      const haystack = `${sp.code} ${sp.name} ${sp.type} ${sp.category} ${sp.gstin} ${sp.state} ${sp.status} ${sp.approval} ${sp.outstanding} ${sp.rating}`.toLowerCase();
      const matchesQuery = !q || haystack.includes(q);
      const matchesStatus = filter === "All" || sp.status === filter;
      const matchesType = typeFilter === "All Types" || sp.type === typeFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted">Manage suppliers, GST, payments, credit &amp; performance in one place.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/masters/supplier/new">
            <Button variant="outline" size="md"><FileCheck2 className="h-4 w-4" /> Upload GST Certificate</Button>
          </Link>
          <Button variant="outline" size="md" onClick={() => setImportOpen(true)}>
            <Upload className="h-4 w-4" /> Import
          </Button>
          <Link href="/settings/documents?screen=supplier_master">
            <Button variant="outline" size="md"><Settings2 className="h-4 w-4" /> Configure Document Field</Button>
          </Link>
          <Link href="/masters/supplier/new">
            <Button size="md"><Plus className="h-4 w-4" /> Add Supplier</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        <SupplierStat icon={Truck} label="Total Suppliers" value={stats.total.toLocaleString()} sub="all vendors" tone="primary" />
        <SupplierStat icon={CheckCircle2} label="Active" value={stats.active.toLocaleString()} sub={`${stats.total ? Math.round(stats.active / stats.total * 100) : 0}% of total`} tone="success" />
        <SupplierStat icon={Ban} label="Blocked" value={String(stats.blocked)} sub="on hold" tone="danger" />
        <SupplierStat icon={Star} label="Preferred Suppliers" value={String(stats.preferred)} sub="key vendors" tone="accent" />
        <SupplierStat icon={Clock} label="Pending Approvals" value={String(stats.pending)} sub="awaiting checker" tone="warning" />
      </div>
        ...
        <div className="flex flex-col gap-3 border-b border-border p-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-1 flex-wrap items-center gap-2">
            <div className="relative min-w-[200px] flex-1 sm:max-w-xs">
              ...
              <input value={query} onChange={(e) => { setQuery(e.target.value); reset(); }} placeholder="Search supplier name, code, GSTIN…" className="h-9 w-full rounded-md border border-border bg-surface-2 pl-9 pr-3 text-sm text-foreground placeholder:text-subtle focus:border-primary focus:bg-surface focus:outline-none focus:shadow-focus" />
            </div>
            <FilterSelect value={branch} onChange={(v) => { setBranch(v); reset(); }} options={BRANCHES} />
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button className="grid h-8 w-8 place-items-center rounded-md border border-border bg-surface text-muted transition hover:border-primary/40 hover:text-primary" aria-label="More filters">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                <tr key={s.id} className="border-b border-border last:border-0 transition hover:bg-primary-subtle/30">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-brand-gradient text-xs font-bold text-white shadow-sm">{s.name.charAt(0)}</span>
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="truncate font-semibold text-foreground">{s.name}</span>
                          {s.preferred && <Star className="h-3.5 w-3.5 shrink-0 fill-accent text-accent" />}
                        </div>
                        <div className="text-2xs text-subtle">{s.code}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-muted">{s.type}</td>
                  <td className="px-4 py-3 font-mono text-2xs text-muted">{s.gstin}</td>
                  <td className="px-4 py-3 text-muted">{s.state}</td>
                  <td className="px-4 py-3 text-right font-bold text-foreground">
                    {s.outstanding > 0 ? fmt.money(s.outstanding) : <span className="text-success">Settled</span>}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className="inline-flex items-center gap-1 rounded-full bg-accent/15 px-2 py-0.5 text-xs font-bold text-accent-foreground">
                      <Star className="h-3 w-3 fill-current" />{s.rating.toFixed(1)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <Badge tone={s.approval === "Approved" ? "success" : s.approval === "Pending" ? "warning" : s.approval === "Rejected" ? "danger" : "neutral"}>{s.approval}</Badge>
                  </td>
                  <td className="px-4 py-3 text-center"><Badge tone={STATUS_TONE[s.status]}>{s.status}</Badge></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <IconAction href={`/masters/supplier/${s.id}`} title="View" icon={Eye} />
                      <IconAction href={`/masters/supplier/${s.id}`} title="Edit" icon={Pencil} accent />
                      <button title="More" className="grid h-8 w-8 place-items-center rounded-md border border-transparent text-muted transition hover:border-border hover:bg-surface-2 hover:text-foreground">
              ...
              {loading && all.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-muted">Loading suppliers…</td></tr>
              )}
              {!loading && rows.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-muted">{all.length === 0 ? <>No suppliers yet. Click <Link href="/masters/supplier/new" className="font-semibold text-primary hover:underline">Add Supplier</Link>.</> : "No suppliers match your search."}</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
const STAT_TONES = {
  ...
  warning: { chip: "bg-warning text-white", sub: "text-muted" },
  danger: { chip: "bg-danger text-white", sub: "text-danger" },
  accent: { chip: "bg-accent text-accent-foreground", sub: "text-muted" },
} as const;

function SupplierStat({ icon: Icon, label, value, sub, tone }: { icon: LucideIcon; label: string; value: string; sub: string; tone: keyof typeof STAT_TONES }) {
...
function ImportModal({ onClose }: { onClose: () => void }) {
  const templates = ["Supplier Master", "Outstanding Balances", "Product Mapping"];
  return (
      ...
      <div className="animate-fade-in flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-bold text-foreground">Import Suppliers</p><p className="text-2xs text-muted">Excel or CSV — bulk add & update</p></div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <div className="space-y-2 rounded-lg border border-border bg-surface p-3 text-sm">
            ...
            <ul className="space-y-1 text-2xs text-muted">
              <li>Row 8 — Duplicate GST number.</li>
              <li>Row 23 — Invalid IFSC code.</li>
            </ul>
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-3">
          <Button variant="outline" size="md" onClick={onClose}>Download Error Report</Button>
          <Button size="md" onClick={onClose}>Import 48 Suppliers</Button>
        </div>
      </div>
    </div>
  );
