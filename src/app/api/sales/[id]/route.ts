...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const setup = await prisma.companySetup.findFirst({
    ...
    select: { companyName: true, companyPhone: true, companyEmail: true, gstNumber: true },
  });
  ...
  return NextResponse.json({
    ...
    business: {
      ...
      phone: setup?.companyPhone || "", email: setup?.companyEmail || "", gstin: setup?.gstNumber || "", address: "",
    },
    template: {
