...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const data: GrnDetail = {
      ...
      transporterName: grn.transporterName ?? "", transportMode: grn.transportMode ?? "", vehicleNo: grn.vehicleNo ?? "",
      ...
      lines: grn.lines.map((l) => ({
        ...
        remarks: l.remarks ?? "", qrMode: l.qrMode as "shared" | "unique", qrStatus: l.qrStatus as "Pending" | "Generated", qrGeneratedCount: l.qrGeneratedCount, printedQty: l.printedQty,
      })),
  };
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (post) {
        ...
        for (const line of grn.lines) {
          const r = await generateLineQr(tx, {
            ...
            productId: line.productId, sku: line.sku, docNo: grn.grnNo, qty: Number(line.qty), mode: line.qrMode === "unique" ? "unique" : "shared",
      ...
      await syncGrnPayable(tx, user.tenantId, { id, grnNo: grn.grnNo, supplier: header.supplier as string | null, grnDate: header.grnDate as string, paymentTerms: header.paymentTerms as string | null, dueDate: header.dueDate as string | null }, grandTotal, amountPaid, post, { businessId: grn.businessId, branchId: grn.branchId });
    });
