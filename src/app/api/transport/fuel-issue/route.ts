...
export async function GET(req: Request) {
  ...
  const tankIds = Array.from(new Set(rows.map((r) => r.tankId)));
  ...
  const list: FuelIssueRow[] = rows.map((r) => ({
    id: r.id, issueNo: r.issueNo, issueDate: dateStr(r.issueDate) ?? "", tankId: r.tankId, tankName: tkMap.get(r.tankId) ?? "—",
    vehicleId: r.vehicleId, vehicleNo: vMap.get(r.vehicleId) ?? "—", driverId: r.driverId, driverName: r.driverId != null ? dMap.get(r.driverId) ?? null : null,
...
export async function POST(req: Request) {
  ...
  const created = await prisma.$transaction(async (tx) => {
    const row = await tx.fuelIssue.create({
      data: {
        ...
        vehicleId: b.vehicleId, driverId: b.driverId ?? null, tripId: b.tripId ?? null, fuelType: tank.fuelType,
  ...
  await writeAudit(prisma, user, { action: "fuel_issue.create", entity: "FuelIssue", entityId: created.id, summary: `Fuel issue ${issueNo} — ${b.quantity}L to vehicle ${vehicle.vehicleNo} from ${tank.tankName}`, businessId: seg.businessId ?? null, branchId: seg.branchId ?? null, ip: requestMeta(req).ip });
  return NextResponse.json({ ok: true, id: created.id, issueNo, warnings, message: "Fuel issued — tank stock updated." }, { status: 201 });
}
