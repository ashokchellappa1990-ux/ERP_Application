...
const EDITOR_TABS = ["General", "Type & Value", "Applicability", "Eligibility", "Validity", "Combination", "Approval", "GST & Accounting"];
...
export function DiscountEditorPage({ id }: { id?: string }) {
  ...
  const [opts, setOpts] = useState<{ categories: string[]; brands: string[]; products: Option[]; customerGroups: string[]; membershipLevels: string[] } | null>(null);
  ...
  useEffect(() => { fetch(`${API}/options`, { cache: "no-store" }).then((r) => r.json()).then((j) => j.ok && setOpts({ categories: j.categories, brands: j.brands, products: j.products, customerGroups: j.customerGroups, membershipLevels: j.membershipLevels })); }, []);
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        {tab === 2 && <div className="space-y-4">
          ...
          <p className="text-2xs text-muted">Leave a field empty to apply to <b>all</b>. Use <b>Select all</b> to target every option. Categories, brands, products, customer groups &amp; levels are loaded live from your masters.</p>
        </div>}
