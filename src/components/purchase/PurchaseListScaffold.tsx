...
import { Plus, Search, Eye, Pencil, MoreVertical, SlidersHorizontal, Settings2, ArrowRight, FileText } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { Pagination } from "@/components/ui/Pagination";
import { PURCHASE_LISTS, PURCHASE_FEATURES, purchaseNotesFor, type PColumn } from "@/lib/purchase/purchaseData";
...
const STATUS_TONE = (s: string): "success" | "warning" | "danger" | "info" | "neutral" => {
  ...
  if (/(paid|accepted|approved|converted|closed|matched|cleared|adjusted|settled|issued|done|passed)/.test(v)) return "success";
  if (/(rejected|mismatch|overdue|hold|excess|short)/.test(v)) return "danger";
  if (/(pending|partial|requested|qc|awaiting|draft hold|on hold)/.test(v)) return "warning";
  if (/(draft|sent|scheduled|quotes|compared|allocated|posted|dispatched)/.test(v)) return "info";
  return "neutral";
};

export function PurchaseListScaffold({ featureKey }: { featureKey: keyof typeof PURCHASE_LISTS }) {
  const fmt = useFmt();
  const inr = (n: number) => fmt.money(n);
  const cfg = PURCHASE_LISTS[featureKey];
  const Icon = PURCHASE_FEATURES.find((f) => f.key === featureKey)?.icon ?? FileText;
  ...
  const cell = (r: Record<string, string | number>, c: PColumn) => {
    const v = r[c.key];
    ...
    if (c.key === "no" || c.key === "supplier") return <span className="font-medium text-foreground">{String(v)}</span>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-muted"><Link href="/purchase" className="hover:text-foreground">Purchase</Link><span className="text-subtle">/</span><span className="font-medium text-foreground">{cfg.title}</span></div>
        ...
        {cfg.newLabel && <Link href={`/purchase/${featureKey}/new`}><Button size="md"><Plus className="h-4 w-4" /> {cfg.newLabel}</Button></Link>}
      </div>
