...
import { Plus, Search, Eye, Pencil, MoreVertical, SlidersHorizontal, Settings2, FileText } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { Pagination } from "@/components/ui/Pagination";
import { INVENTORY_LISTS, INVENTORY_FEATURES, INVENTORY_FORM_META, invNotesFor, type IColumn } from "@/lib/inventory/inventoryData";
...
const STATUS_TONE = (s: string): "success" | "warning" | "danger" | "info" | "neutral" => {
  ...
  if (/(active|received|completed|approved|healthy|in stock|occupied|fulfilled|sold)/.test(v)) return "success";
  if (/(expired|rejected|overdue|defective|out)/.test(v)) return "danger";
  if (/(near|low|pending|transit|counting|variance|partially|reserved|near full|quarantined|returned)/.test(v)) return "warning";
  if (/(draft|dispatched|scheduled|empty|overstock)/.test(v)) return "info";
  return "neutral";
};

export function InventoryListScaffold({ featureKey }: { featureKey: keyof typeof INVENTORY_LISTS }) {
  const fmt = useFmt();
  const cfg = INVENTORY_LISTS[featureKey];
  const Icon = INVENTORY_FEATURES.find((f) => f.key === featureKey)?.icon ?? FileText;
  const hasForm = !!INVENTORY_FORM_META[featureKey];
  ...
  const cell = (r: Record<string, string | number>, c: IColumn) => {
    const v = r[c.key];
    ...
    if (c.key === "no" || c.key === "code" || c.key === "product" || c.key === "name" || c.key === "branch" || c.key === "batch" || c.key === "serial" || c.key === "bin") return <span className="font-medium text-foreground">{String(v)}</span>;
    return <span className={cn(typeof v === "number" && v < 0 ? "font-semibold text-danger" : "text-muted")}>{String(v)}</span>;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-muted"><Link href="/inventory" className="hover:text-foreground">Inventory</Link><span className="text-subtle">/</span><span className="font-medium text-foreground">{cfg.title}</span></div>
        ...
        {cfg.newLabel && hasForm && <Link href={`/inventory/${featureKey}/new`}><Button size="md"><Plus className="h-4 w-4" /> {cfg.newLabel}</Button></Link>}
      </div>
