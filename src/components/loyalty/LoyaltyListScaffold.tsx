...
import { LOYALTY_LISTS, LOYALTY_FEATURES, LOYALTY_FORM_META, LOYALTY_GUIDES, loyaltyNotesFor, type LColumn } from "@/lib/loyalty/loyaltyData";
...
const STATUS_TONE = (s: string): "success" | "warning" | "danger" | "info" | "neutral" => {
  ...
  if (/(active|approved|live|redeemed|profit)/.test(v)) return "success";
  if (/(rejected|expired|blocked|lapsed|inactive|ended)/.test(v)) return "danger";
  if (/(pending|scheduled|at-risk|frozen|expiring)/.test(v)) return "warning";
  if (/(draft)/.test(v)) return "info";
  return "neutral";
};

export function LoyaltyListScaffold({ featureKey }: { featureKey: keyof typeof LOYALTY_LISTS }) {
  const fmt = useFmt();
  const cfg = LOYALTY_LISTS[featureKey];
  const Icon = LOYALTY_FEATURES.find((f) => f.key === featureKey)?.icon ?? FileText;
  const hasForm = !!LOYALTY_FORM_META[featureKey];
  ...
  const editHref = `/loyalty/${featureKey}/new`;

  const cell = (r: Record<string, string | number>, c: LColumn) => {
    const v = r[c.key];
    ...
    if (c.kind === "amount") return <span className="font-semibold text-foreground">{typeof v === "number" ? fmt.money(v) : v}</span>;
    if (["no", "code", "name", "customer"].includes(c.key)) return <span className="font-medium text-foreground">{String(v)}</span>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-muted"><Link href="/loyalty" className="hover:text-foreground">Loyalty</Link><span className="text-subtle">/</span><span className="font-medium text-foreground">{cfg.title}</span></div>
        ...
        {cfg.newLabel && hasForm && <Link href={editHref}><Button size="md"><Plus className="h-4 w-4" /> {cfg.newLabel}</Button></Link>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {cfg.kpis.map((k) => (
          <div key={k.label} className="rounded-2xl border border-border bg-card p-3.5 shadow-sm">
            <div className="flex items-center gap-2.5"><span className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-lg text-sm font-bold shadow-sm", STAT_TONES[k.tone])}>{k.value.replace(/[^0-9]/g, "").slice(0, 1) || "•"}</span><p className="text-base font-bold tracking-tight text-foreground">{k.value}</p></div>
              ...
              {paged.map((r, i) => (
                <tr key={i} className="border-b border-border last:border-0 transition hover:bg-primary-subtle/30">
                  ...
                  <td className="px-4 py-3"><div className="flex items-center justify-end gap-1">
                    ...
                    {hasForm ? <Link href={editHref} title="Edit" className="grid h-8 w-8 place-items-center rounded-md border border-primary/30 bg-primary-subtle text-primary transition hover:bg-primary hover:text-white"><Pencil className="h-4 w-4" /></Link> : <button onClick={() => setViewRow(r)} title="Details" className="grid h-8 w-8 place-items-center rounded-md border border-border bg-surface text-muted transition hover:border-primary/40 hover:text-primary"><Pencil className="h-4 w-4" /></button>}
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center p-0 sm:items-center sm:p-4">
          ...
          <div className="relative z-10 flex max-h-[88vh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl border border-border bg-card shadow-2xl sm:rounded-2xl">
            <div className="flex items-center gap-2.5 border-b border-border bg-primary-subtle/40 px-5 py-3.5">
              ...
              <button type="button" onClick={() => setViewRow(null)} className="grid h-8 w-8 place-items-center rounded-lg text-muted transition hover:bg-surface-2 hover:text-foreground"><X className="h-4 w-4" /></button>
            </div>
              ...
              <dl className="divide-y divide-border">
                {cfg.columns.map((c) => (
                  <div key={c.key} className="flex items-center justify-between gap-4 py-2">
                    ...
                    <dd className="text-right text-sm">{c.kind === "status" ? <Badge tone={STATUS_TONE(String(viewRow[c.key]))}>{String(viewRow[c.key])}</Badge> : <span className="font-semibold text-foreground">{c.kind === "amount" && typeof viewRow[c.key] === "number" ? fmt.money(viewRow[c.key] as number) : String(viewRow[c.key] ?? "—")}</span>}</dd>
                  </div>
                ))}
              </dl>
            </div>
