...
export function StationTankManager({ tanks, onChanged }: { tanks: TankRow[]; onChanged: () => void }) {
  ...
  return (
    <div className="space-y-6">
      {/* Fuel Stations management hidden — vendor/supplier (via the "+" quick-add
          on Fuel Purchase) now covers this, and Fuel Tank creation no longer
          depends on a Station. Re-enable by uncommenting if a formal Station
          master is needed again; StationModal is left intact below. */}
...
function TankModal({ id, stations, onClose, onSaved }: { id?: number; stations: StationRow[]; onClose: () => void; onSaved: () => void }) {
  const toast = useToast();
  const [f, setF] = useState<TankInput>({ tankCode: "", tankName: "", stationId: null, fuelType: "Diesel", capacity: 0, minLevel: null, maxLevel: null, status: "Active", remarks: "" });
  const [busy, setBusy] = useState(false);
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid gap-3 p-5 sm:grid-cols-2">
          ...
          <div><label className={lbl}>Station (optional)</label><select value={f.stationId ?? ""} onChange={(e) => set("stationId", Number(e.target.value) || null)} className={inp}><option value="">— None —</option>{stations.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
