          ...
          <div className="overflow-x-auto">
                      ...
                      <Frag key={g.party}>
                        ...
                        {open && <tr><td colSpan={7} className="bg-surface-2/40 px-4 py-2">
                          ...
                          <tbody>{invoices.map((p) => (
                            <tr key={`${p.source ?? "sale"}-${p.id}`} className="border-t border-border/60"><td className="py-1.5">{p.source === "dispatch" ? <Link href={`/warehouse/transfer/load-dispatch/${p.loadDispatchId}`} className="font-mono text-primary hover:underline">{p.refNo}</Link> : <Link href={p.channel === "B2B" ? `/sales/invoice/${p.id}` : "/sales/history"} className="font-mono text-primary hover:underline">{p.refNo}</Link>} <span className="text-subtle">{p.channel}</span></td><td className="py-1.5"><span className="text-foreground">{p.docDate || "—"}</span> · <span className={cn(p.overdue ? "font-semibold text-danger" : "text-subtle")}>due {p.dueDate || "—"}</span></td><td className="py-1.5 text-right text-muted">{inr(p.totalAmount)}</td><td className="py-1.5 text-right text-muted">{inr(p.paidAmount)}</td><td className="py-1.5 text-right font-semibold text-foreground">{inr(p.balanceAmount)}</td><td className="py-1.5 text-center">{p.source === "dispatch" ? <Badge tone="neutral">Pending Invoice</Badge> : <Badge tone={p.overdue && p.status !== "Paid" ? "danger" : TONE[p.status] ?? "neutral"}>{p.overdue && p.status !== "Paid" ? "Overdue" : p.status}</Badge>}</td><td className="py-1.5 text-right">{p.source === "dispatch" ? <Link href={`/warehouse/transfer/load-dispatch/${p.loadDispatchId}`} className="rounded border border-border-strong bg-surface px-2 py-0.5 font-semibold text-muted hover:text-foreground">Post Invoice</Link> : p.balanceAmount > 0 && <button onClick={() => setCollect(p)} className="rounded border border-primary/30 bg-primary-subtle px-2 py-0.5 font-semibold text-primary hover:bg-primary hover:text-white">Collect</button>}</td></tr>
            ...
            {view === "invoice" && (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((p) => (
                    <tr key={`${p.source ?? "sale"}-${p.id}`} className="border-b border-border last:border-0 hover:bg-primary-subtle/20">
                      ...
                      <td className="px-4 py-3">{p.source === "dispatch" ? <Link href={`/warehouse/transfer/load-dispatch/${p.loadDispatchId}`} className="font-mono text-2xs text-primary hover:underline">{p.refNo}</Link> : <Link href={p.channel === "B2B" ? `/sales/invoice/${p.id}` : "/sales/history"} className="font-mono text-2xs text-primary hover:underline">{p.refNo}</Link>}<div className="text-2xs text-subtle">{p.channel}</div></td>
                      ...
                      <td className="px-4 py-3 text-center">{p.source === "dispatch" ? <Badge tone="neutral">Pending Invoice</Badge> : <Badge tone={p.overdue && p.status !== "Paid" ? "danger" : TONE[p.status] ?? "neutral"}>{p.overdue && p.status !== "Paid" ? "Overdue" : p.status}</Badge>}</td>
                      <td className="px-4 py-3 text-right">{p.source === "dispatch" ? <Link href={`/warehouse/transfer/load-dispatch/${p.loadDispatchId}`}><Button size="sm" variant="outline">Post Invoice</Button></Link> : p.balanceAmount > 0 && <Button size="sm" variant="outline" onClick={() => setCollect(p)}><Wallet className="h-3.5 w-3.5" /> Collect</Button>}</td>
                    </tr>
                  ))}
