...
import { Building2, Plus, Pencil, Eye, Power, Trash2, X, CornerDownRight } from "lucide-react";
...
import type { CostCentreRow, CostCentreDto } from "@/lib/contracts/costCentre";

interface Branch { id: number; name: string }
const today = () => new Date().toISOString().slice(0, 10);
type Form = Omit<CostCentreDto, "id" | "code" | "branchName" | "parentName" | "businessId">;
const BLANK: Form = { branchId: null, name: "", shortName: "", description: "", department: "", parentId: null, allowChildren: true, manager: "", budgetApplicable: false, status: "Active", effectiveFrom: today(), effectiveTo: null, remarks: "" };
...
export function CostCentreMaster() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const j = await fetch("/api/finance/cost-centre", { cache: "no-store" }).then((r) => r.json()).catch(() => null);
  ...
  const tree = useMemo(() => {
    const byParent = new Map<number | null, CostCentreRow[]>();
    for (const r of rows) { const k = r.parentId; if (!byParent.has(k)) byParent.set(k, []); byParent.get(k)!.push(r); }
    const out: { row: CostCentreRow; depth: number }[] = [];
    const walk = (parentId: number | null, depth: number) => { for (const r of byParent.get(parentId) ?? []) { out.push({ row: r, depth }); walk(r.id, depth + 1); } };
    walk(null, 0);
    // include orphans (parent filtered out of scope) at root
    const seen = new Set(out.map((x) => x.row.id));
    for (const r of rows) if (!seen.has(r.id)) out.push({ row: r, depth: 0 });
    return out;
  }, [rows]);

  async function toggle(r: CostCentreRow) {
    const j = await fetch(`/api/finance/cost-centre/${r.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: r.status === "Active" ? "Inactive" : "Active" }) }).then((x) => x.json());
    if (j.ok) { toast.show(j.message || "Updated.", { type: "success" }); load(); } else toast.show(j.message || "Failed.", { type: "error" });
  }
  async function remove(r: CostCentreRow) {
    if (!window.confirm(`Delete ${r.code} — ${r.name}?`)) return;
    const j = await fetch(`/api/finance/cost-centre/${r.id}`, { method: "DELETE" }).then((x) => x.json());
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-muted"><span>System</span><span className="text-subtle">/</span><span>Organization Configuration</span><span className="text-subtle">/</span><span className="font-medium text-foreground">Cost Centre</span></div>
          <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-foreground"><Building2 className="h-5 w-5 text-primary" /> Cost Centre Master</h1>
          <p className="mt-0.5 text-sm text-muted">Financial dimensions that track &amp; allocate expenses — supports unlimited hierarchy. Captured on postings for dimensional reporting.</p>
        </div>
        <Button size="md" onClick={() => setModal({ mode: "add" })}><Plus className="h-4 w-4" /> Add Cost Centre</Button>
      </div>

      {loading ? <div className="rounded-2xl border border-border bg-card p-10 shadow-sm"><AppLoader label="Loading…" size="sm" /></div>
        : rows.length === 0 ? <div className="rounded-2xl border border-dashed border-border-strong bg-card p-10 text-center text-sm text-muted">No cost centres yet. <button onClick={() => setModal({ mode: "add" })} className="font-semibold text-primary hover:underline">Add one →</button></div>
        : (
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm"><div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-sm">
            <thead><tr className="border-b border-border bg-surface-2/40 text-2xs uppercase tracking-wide text-muted"><th className="px-3 py-2.5 text-left">Code</th><th className="px-3 py-2.5 text-left">Cost Centre</th><th className="px-3 py-2.5 text-left">Department</th><th className="px-3 py-2.5 text-left">Manager</th><th className="px-3 py-2.5 text-center">Budget</th><th className="px-3 py-2.5 text-center">Status</th><th className="px-3 py-2.5 text-right">Actions</th></tr></thead>
            <tbody>
              {tree.map(({ row: r, depth }) => (
                <tr key={r.id} className="border-b border-border/60 last:border-0 hover:bg-surface-2/20">
                  ...
                  <td className="px-3 py-2 text-2xs text-muted">{r.department ?? "—"}</td>
                  <td className="px-3 py-2 text-2xs text-muted">{r.manager ?? "—"}</td>
                  <td className="px-3 py-2 text-center">{r.budgetApplicable ? <Badge tone="info">Yes</Badge> : <span className="text-2xs text-subtle">No</span>}</td>
      ...
      {modal && <CostCentreModal mode={modal.mode} id={modal.id} branches={branches} parents={rows} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />}
    </div>
  );
}

function CostCentreModal({ mode, id, branches, parents, onClose, onSaved }: { mode: "add" | "edit" | "view"; id?: number; branches: Branch[]; parents: CostCentreRow[]; onClose: () => void; onSaved: () => void }) {
  const toast = useToast();
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`/api/finance/cost-centre/${id}`, { cache: "no-store" }).then((r) => r.json()).then((j) => { if (j.ok) { const c: CostCentreDto = j.costCentre; setF({ branchId: c.branchId, name: c.name, shortName: c.shortName ?? "", description: c.description ?? "", department: c.department ?? "", parentId: c.parentId, allowChildren: c.allowChildren, manager: c.manager ?? "", budgetApplicable: c.budgetApplicable, status: c.status, effectiveFrom: c.effectiveFrom ?? "", effectiveTo: c.effectiveTo ?? "", remarks: c.remarks ?? "" }); setCode(c.code); } setLoading(false); }).catch(() => setLoading(false));
  }, [id]);

  async function save() {
    if (!f.name.trim()) { toast.show("Enter a name.", { type: "error" }); return; }
    setBusy(true);
    const j = await fetch(id ? `/api/finance/cost-centre/${id}` : "/api/finance/cost-centre", { method: id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(f) }).then((r) => r.json());
    setBusy(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-3"><h3 className="text-base font-bold text-foreground">{mode === "add" ? "Add" : mode === "view" ? "View" : "Edit"} Cost Centre{code ? ` · ${code}` : ""}</h3><button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-md text-muted hover:bg-surface-2"><X className="h-4 w-4" /></button></div>
        <div className="max-h-[70vh] overflow-y-auto px-5 py-4">
          {loading ? <AppLoader label="Loading…" size="sm" /> : (
            <fieldset disabled={readOnly} className="grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2"><label className={lbl}>Cost Centre Name *</label><input value={f.name} onChange={(e) => set("name", e.target.value)} className={inp} /></div>
              <div><label className={lbl}>Short Name</label><input value={f.shortName ?? ""} onChange={(e) => set("shortName", e.target.value)} className={inp} /></div>
              <div><label className={lbl}>Department</label><input value={f.department ?? ""} onChange={(e) => set("department", e.target.value)} className={inp} /></div>
              ...
              <div><label className={lbl}>Parent Cost Centre</label><select value={f.parentId ?? ""} onChange={(e) => set("parentId", Number(e.target.value) || null)} className={inp}><option value="">— None (top level) —</option>{parents.filter((p) => p.id !== id && p.allowChildren).map((p) => <option key={p.id} value={p.id}>{p.code} · {p.name}</option>)}</select></div>
              ...
              <div><label className={lbl}>Effective From</label><input type="date" value={f.effectiveFrom ?? ""} onChange={(e) => set("effectiveFrom", e.target.value)} className={inp} /></div>
              <div><label className={lbl}>Effective To</label><input type="date" value={f.effectiveTo ?? ""} onChange={(e) => set("effectiveTo", e.target.value || null)} className={inp} /></div>
              <label className="flex items-center justify-between gap-2 rounded-md border border-border bg-surface px-3 py-2 text-sm"><span>Allow Child Cost Centres</span><Switch checked={f.allowChildren} onChange={(v) => set("allowChildren", v)} aria-label="allow" /></label>
              <label className="flex items-center justify-between gap-2 rounded-md border border-border bg-surface px-3 py-2 text-sm"><span>Budget Applicable</span><Switch checked={f.budgetApplicable} onChange={(v) => set("budgetApplicable", v)} aria-label="budget" /></label>
