...
import type { SalesReturnRow as ReturnRow, SalesReturnListStats as Stats } from "@/lib/contracts/salesReturn";

const EMPTY: Stats = { total: 0, pending: 0, refundValue: 0 };
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`/api/sales/returns?${u}`, { cache: "no-store", signal: ctrl.signal });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-muted"><span>Sales</span><span className="text-subtle">/</span><span className="font-medium text-foreground">Sales Returns</span></div>
          <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-foreground"><Undo2 className="h-5 w-5 text-primary" /> Sales Returns</h1>
          <p className="mt-0.5 text-sm text-muted">Returns &amp; refunds raised against sales invoices.</p>
        </div>
        <Link href="/sales/return/new"><Button size="md"><Plus className="h-4 w-4" /> New Return</Button></Link>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Stat icon={IndianRupee} label="Refund Value" value={inr(stats.refundValue)} tone="success" />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative min-w-[200px] flex-1 sm:max-w-xs">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search return, invoice or customer…" className="h-9 w-full rounded-md border border-border bg-surface-2 pl-9 pr-3 text-sm text-foreground placeholder:text-subtle focus:border-primary focus:bg-surface focus:outline-none focus:shadow-focus" />
          </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-border last:border-0 transition hover:bg-primary-subtle/30">
                  <td className="px-4 py-3"><Link href={`/sales/return/${r.id}`} className="font-mono text-xs font-semibold text-primary hover:underline">{r.returnNo}</Link></td>
                  ...
                  <td className="px-4 py-3 text-muted">{r.refundMode || "—"}</td>
                  ...
                  <td className="px-4 py-3"><div className="flex items-center justify-end">
                    <Link href={`/sales/return/${r.id}`} title="View" className="grid h-8 w-8 place-items-center rounded-md border border-border bg-surface text-muted transition hover:border-primary/40 hover:text-primary"><Eye className="h-4 w-4" /></Link>
                  </div></td>
                </tr>
              ))}
              {loading && rows.length === 0 && <tr><td colSpan={9} className="px-4 py-8"><AppLoader label="Loading returns…" size="sm" /></td></tr>}
              {!loading && rows.length === 0 && <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-muted">{notAuthed ? <>Please <Link href="/login" className="font-semibold text-primary hover:underline">sign in</Link>.</> : <>No sales returns yet. Click <Link href="/sales/return/new" className="font-semibold text-primary hover:underline">New Return</Link> to create one.</>}</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
